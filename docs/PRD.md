...
| 以图搜图 | P1 | ⚠️ API 有 | 后端 `POST /search/by-image`；App 未接 |
| 向量检索 | P1 | ❌ | Qdrant 已部署，未写入向量 |
| 充电/借出提醒 API | P1 | ✅ | Beat 扫描；**无 FCM 推送** |
| 借出标记 UI | P1 | ❌ | 仅提醒页可「已归位」 |
...
| 版本 | 目标 | 内容 |
|------|------|------|
| **V1.0.11–1.0.12（当前）** | 生产可用 | 识别修复、搜索索引、category、空间浏览、充电提醒自动创建 |
| V1.1 | 体验 | 真实语音、以图搜图 App、FCM、借出 UI、搜索跳转空间 |
